  ...
  return (
    <section id="prise" >
      <div className="container overflow-x-hidden pb-[40px] pt-[50px] lg:py-[60px]">
        ...
        >
          <h2 className="relative text-center uppercase leading-[31px] font-extrabold text-red text-[84px] sm:leading-[35px] sm:text-[90px] text-opacity-70 mb-[80px] md:w-[500px] md:mx-auto md:leading-[59px] md:text-[130px] md:mb-[100px] lg:text-[200px] lg:mb-[90px] lg:w-[700px] lg:pt-[40px] lg:leading-[75px]">
            наші{" "}
            <motion.div
              viewport={{ once: true }}
        ...
        <ul className="flex flex-wrap gap-y-[80px] gap-x-0 g mb-8 md:gap-x-[50px] md:gap-y-[80px] lg:gap-x-8 lg:gap-y-[90px]">
          {price.map(({ id, title, src, description, price }) => (
            <li
              key={id}
              className={`${isPriceItem && id > 4 ? 'hidden__item' : ''} mx-auto px-3 relative pt-[80px] text-center bg-lightgrey rounded-3xl w-[250px] h-[250px] sm:px-4 lg:w-[350px] lg:pt-[90px] bg:h-[300px]`}
            >
              <Image
                src={src}
                alt="nail"
